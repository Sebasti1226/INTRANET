...
function initEscalacion() {
  ...
  btnEscalacionLoad.addEventListener("click", () => {
    ...
    const filas = escalacionTabla.querySelectorAll("tr");
  ...
  btnEscalacionCopiar.addEventListener("click", () => {
    ...
    filas.forEach(fila => {
      ...
      if (th && td) {
        textoCompleto += `${th.textContent}: ${td.textContent}\n`;
      }
    });
    ...
    navigator.clipboard.writeText(textoCompleto)
      .then(() => alert("✅ Tabla copiada como texto al portapapeles"))
      .catch(err => {
        console.error("Error copiando:", err);
